...
<template>
  <van-cell-group inset class="!mt-2">
    <van-cell valueClass="flex justify-end" titleClass="h-9 yCenter !flex-none" class="!px-2">
      <template #title>
    ...
    <div class="flex bg-[#ffddb3] m-3 rounded justify-between px-8 text-white h-8 yCenter">
      <div>已并网</div>
      <div>
        {{ previewData.find((n) => n.title == "已并网")?.value }}
      </div>
    </div>
    <div class="grid grid-cols-2 px-8">
      <div v-for="p of previewData.slice(0)" :key="p.title">
        <div>
          {{ p.title }}
        </div>
        <div>
