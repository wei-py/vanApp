<script setup>
import { goods, exportImages, deviceInfo, deviceContent } from "./goodsRejectDetail";
...
async function getData() {
  const { data } = await http.get(`sto/stoBack/show/back-device-backId/${query.backId}`)
  console.log(data)
        ...
        <van-tab v-for="t in slot.tabs" :title="t.title" :name="t.name" :key="t.name">
          <vantForm :form="_.deviceContent" class="m-0">
            <template #button="{ slot }">
              <div v-if="!slot.hidden" class="grid grid-cols-3 gap-2 p-2">
