...
const dic = ref({
  FIRST_TO_SECOND: "二级仓库退一级仓库",
  FIRST_TO_SITE: "现场退一级仓库",
  SECOND_TO_SITE: "现场退二级仓库",
  SITE: "现场退货",
});
...
async function getData() {
  const body = {
    ...query,
    queryTag: search.value,
    state: tab.value + 2,
  };
  const { data } = await http.post(queryUrl(`sto/stoBack/page`, page.value), body);
...
function onShowDetail(item) {
  router.push({
    ...
    query: {
      title: dic.value[item.stoBack.backType],
      backId: item.stoBack?.backId || "",
      state: tab.value,
      backType: item.stoBack.backType,
    },
  });
}
        ...
        <template v-for="item in list" :key="item.index">
          ...
          <van-cell-group inset class="px-2 py-4 !mb-2 shadowC !rounded-xl" v-skeleton="flag.overlayShow">
             <van-cell
              ...
              :border="false"
              title="退货编号: "
              :value="item.stoBack.backId || '-'"
            >
              <template #right-icon>
                <plainButton class="!h-[24px]" @click.stop="$copyText(item.stoBack.backId)"> 复制 </plainButton>
              </template>
            </van-cell>
            <van-cell
            ...
            <van-cell
              ...
              :border="false"
              title="退货类型: "
              :value="item.stoBack.backType && dic[item.stoBack.backType] ? dic[item.stoBack.backType] : '-'"
            />
            <!-- <van-cell
              valueClass="valueClass"
              titleClass="titleClass"
            ...
            /> -->
            <van-cell valueClass="valueClass" titleClass="titleClass" :border="false" title="备注: " :value="item.stoBack.remark || '-'" />
            <van-cell :border="false" title="">
              <vButton @click="onShowDetail(item)"> 退货详情 </vButton>
            </van-cell> 
          </van-cell-group>
        </template>
      </van-list>
